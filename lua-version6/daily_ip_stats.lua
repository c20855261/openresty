-- daily_ip_stats.lua (v3.4.3 with minimal enhancements)
-- 功能：
--  - 顯示 daily ip stats，管理 whitelist/ban，支援 geo_mapping 中文顯示（fallback ip-api）
--  - 最小變更：改為優先使用 per-IP daily keys (daily:YYYYMMDD:<ip>) 的 JSON 值作為來源，
--    並在顯示區以五個欄位呈現： last_ban, error_ban, conn_ban, ip, geo
--  - 保留原有的白名單/黑名單管理、ops log 與頁面功能
--  - 使用 SCAN 取得 daily keys（避免 KEYS 阻塞）
...
local DEFAULT_TOP = 25
local DATE_RANGE_DAYS = 7
local OPS_LOG_LIST = "ops_log"
local OPS_LOG_MAX = 10
local ADMIN_TOKEN = "" 
...
if ngx.req.get_method() == "POST" then
    ...
    elseif action == "remove_blacklist" then
        ...
        if red then
            ...
            local d3 = red:del(notify_key)
            total_deleted = total_deleted + (tonumber(d3) or 0)
            red:set_keepalive(10000,100)
        end
...
---------------------------------------------------------------------
-- GET handler: 使用 per-IP daily keys 作為資料來源並映射為五欄位
---------------------------------------------------------------------
...
-- 新：collect per-ip daily keys using scan pattern "daily:YYYYMMDD:*"
local function collect_daily_perip_keys(date_str)
    local keys = {}
    local pattern = "daily:" .. date_str .. ":*"
    local cursor = "0"
    repeat
        local r = red:scan(cursor, "MATCH", pattern, "COUNT", 200)
        if not r then
            -- fallback to KEYS (only if scan fails)
            local kres = red:keys(pattern)
            if kres and type(kres) == "table" then
                for _, k in ipairs(kres) do table.insert(keys, k) end
            end
            break
        end
        cursor = tostring(r[1])
        local arr = r[2] or {}
        for _, k in ipairs(arr) do table.insert(keys, k) end
    until cursor == "0"
    return keys
end

-- parse per-ip JSON key into entry with required five fields
local function parse_daily_perip_entry(key, date_str)
    local entry = {
        last_ban = "",
        error_ban = 0,
        conn_ban = 0,
    ...
    local v = red:get(key)
    if not v or v == ngx.null then return entry end
    local ok, t = pcall(cjson.decode, v)
    if not ok or type(t) ~= "table" then return entry end
    entry.raw = t
    entry.last_ban = t.last_ban or t.lastBan or ""
    entry.error_ban = tonumber(t.error_ban) or tonumber(t.errorBan) or 0
    entry.conn_ban = tonumber(t.conn_ban) or tonumber(t.connBan) or 0
    entry.geo = t.geo or t.region or "Unknown"
    entry.ip = t.ip or key:match("^daily:" .. date_str .. ":(.+)$") or ""
    entry.ip = (entry.ip and entry.ip:match("^%s*(.-)%s*$")) or ""
    return entry
end

-- get top entries based on error_ban desc then conn_ban desc then last_ban
local function get_top_perip(date_str, top_n)
    local keys = collect_daily_perip_keys(date_str)
    local out = {}
    for _, k in ipairs(keys) do
        local e = parse_daily_perip_entry(k, date_str)
        if e.ip and e.ip ~= "" then table.insert(out, e) end
    end
    table.sort(out, function(a,b)
        if (a.error_ban or 0) ~= (b.error_ban or 0) then
            return (a.error_ban or 0) > (b.error_ban or 0)
        end
        if (a.conn_ban or 0) ~= (b.conn_ban or 0) then
            return (a.conn_ban or 0) > (b.conn_ban or 0)
        end
        return (a.last_ban or "") > (b.last_ban or "")
    end)
    if #out > top_n then
        for i = #out, top_n + 1, -1 do table.remove(out, i) end
    end
    return out
end

-- obtain results from per-ip keys
local rows = get_top_perip(date, top_n)

-- 讀取 whitelist, ops, ban list 等（保留原有行為）
...
-- 若 rows 中沒有 region 值，嘗試用 geo lookup（保留原邏輯）
for _, r in ipairs(rows) do
    if not r.geo or r.geo == "" or r.geo == "Unknown" then
        r.geo = get_geo_info_for_ip(red, r.ip) or "unknown"
    end
end
...
ngx.say([[
<style>
body{font-family:Arial,Helvetica,sans-serif;margin:18px;color:#333}
...
.notice{padding:10px;background:#f8f8f8;border:1px solid #eee;margin-top:8px;border-radius:4px}
.ops{margin-top:12px;border:1px solid #eee;padding:12px;background:#fafafa;border-radius:6px}
.small{font-size:0.9em;color:#666}
.msg-success { background:#e9f7ef;border-left:4px solid #34a853;padding:10px;margin:10px 0;border-radius:4px }
.msg-error   { background:#fff0f0;border-left:4px solid #e53935;padding:10px;margin:10px 0;border-radius:4px }
.ip-list{max-height:180px;overflow-y:auto;padding:8px;background:#fff;border:1px solid #ddd;border-radius:4px;font-family:monospace}
table{border-collapse:collapse;width:100%;margin-top:12px;border-radius:6px;overflow:hidden}
th,td{padding:10px;text-align:left;border-bottom:1px solid #eee;background:#fff}
th{background:#f7f9fb;color:#333;font-weight:600;text-transform:uppercase;font-size:0.85em}
.rank{width:60px}
...
ngx.say("<div class='header'><h2>Daily IP Stats for " .. html_escape(date) ..
        " (top " .. tostring(top_n) .. ")</h2>")
ngx.say("<div class='small'>Generated: " .. os.date("%Y-%m-%d %H:%M:%S") .. "</div></div>")
...
ngx.say("<div class='card'><h3>Manage IP (Whitelist / Blacklist)</h3>")
...
-- Whitelist and Blacklist display (left/right)
ngx.say("<div style='display:flex;gap:18px;margin-top:12px'>")
ngx.say("<div style='flex:1' class='card'><h4>Whitelist ("..tostring(#whitelist_list).." IPs)</h4>")
...
ngx.say("<div style='flex:1' class='card'><h4>Blacklist / Banned IPs ("..tostring(#ban_list).." IPs)</h4>")
if #ban_list == 0 then
...
ngx.say("</div></div>") -- end lists

-- Results table: show five fields (Last Ban, Error Ban, Conn Ban, IP, Geo)
ngx.say("<div class='card'><h3>Results</h3>")
ngx.say("<table><thead><tr><th class='rank'>Rank</th><th>Last Ban</th><th>Error Ban</th><th>Conn Ban</th><th>IP</th><th>Geo</th></tr></thead><tbody>")
if #rows == 0 then
    ngx.say("<tr><td colspan='6' style='text-align:center;padding:18px'>No data</td></tr>")
else
    for idx, r in ipairs(rows) do
        ngx.say("<tr>")
        ngx.say("<td class='rank'>"..tostring(idx).."</td>")
        ngx.say("<td>"..html_escape(r.last_ban or "").."</td>")
        ngx.say("<td class='count'>"..tostring(r.error_ban or 0).."</td>")
        ngx.say("<td class='count'>"..tostring(r.conn_ban or 0).."</td>")
        ngx.say("<td>"..html_escape(r.ip or "").."</td>")
        ngx.say("<td>"..html_escape(r.geo or "").."</td>")
        ngx.say("</tr>")
    end
end
...
-- Ops log
ngx.say("<div class='ops'><h4>Recent Operations</h4><div class='small'><pre style='white-space:pre-wrap'>")
if #ops == 0 then
    ngx.say("（無操作紀錄）")
else
    for _, l in ipairs(ops) do
        ngx.say(html_escape(cjson.encode(l)) .. "\n")
    end
end
ngx.say("</pre></div></div>")

ngx.say("</div></body></html>")
